...
import { motion, AnimatePresence, useScroll } from "framer-motion"; // ← Added AnimatePresence
import { useState, useEffect, useRef } from "react";

// ADDED MISSING useInView hook
...
// Magnetic link component for interactive hover effects
const MagneticSocialLink = ({ href, icon: Icon, label, color, hoverColor }) => {
  ...
  return (
    <motion.a
      ...
      className="group relative flex items-center justify-center w-12 h-12 rounded-xl bg-slate-800/50 border border-slate-700/50 backdrop-blur-sm overflow-hidden transition-colors hover:border-slate-600"
    ...
    >
      ...
      <Icon
        className="relative z-10 w-5 h-5 text-slate-400 group-hover:text-white transition-colors duration-300"
    ...
    <div className="max-w-md w-full">
      ...
      <form onSubmit={handleSubmit} className="relative">
        <input
          ...
          className="w-full px-4 py-3 pr-32 rounded-xl bg-slate-800/50 border border-slate-700 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20 transition-all"
          disabled={status === "loading" || status === "success"}
        />
        <motion.button
          type="submit"
          disabled={status === "loading" || status === "success"}
          className={`absolute right-2 top-1/2 -translate-y-1/2 px-4 py-2 rounded-lg font-medium text-sm transition-all ${
            status === "success"
            ...
            <div>
              ...
              <ul className="space-y-3 text-sm text-slate-400">
                <li className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg bg-slate-800/50 flex items-center justify-center text-blue-400">
                    <FaWhatsapp size={14} />
                  </span>
                  +234 902 664 2320
                </li>
                <li className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg bg-slate-800/50 flex items-center justify-center text-purple-400">
                    @
                  </span>
                  abdullahi@example.com
                </li>
                <li className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg bg-slate-800/50 flex items-center justify-center text-cyan-400">
                    📍
                  </span>
                  Nigeria
                </li>
              ...
              <div className="flex gap-2">
                {socialLinks.map((link) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-8 h-8 rounded-lg bg-slate-800/30 hover:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-white transition-all"
